...
if(NOT CONFIG_CHIP_DEVICE_GENERATE_ROTATING_DEVICE_UID)
    if(NOT CONFIG_CHIP_DEVICE_ROTATING_DEVICE_UID)
...
# check if one of possible ways for appending certificates is set
if(NOT(CONFIG_CHIP_FACTORY_DATA_USE_DEFAULTS_CERTS) AND 
  (NOT CONFIG_CHIP_FACTORY_DATA_GENERATE_DAC_CERT AND NOT CONFIG_CHIP_FACTORY_DATA_GENERATE_PAI_CERT) AND
  (NOT CONFIG_CHIP_DEVICE_PAI_CERT_PATH AND NOT CONFIG_CHIP_DEVICE_DAC_CERT_PATH AND NOT CONFIG_CHIP_DEVICE_DAC_KEY_PATH))
    message(FATAL_ERROR "There is not configuration to include certifications to factory data.
One from following configs must be set:
- CONFIG_CHIP_FACTORY_DATA_USE_DEFAULTS_CERTS to use  pre-generated certificates
- CONFIG_CHIP_FACTORY_DATA_GENERATE_DAC_CERT=y and CONFIG_CHIP_FACTORY_DATA_GENERATE_PAI_CERT=y to generate new certificates
- CONFIG_CHIP_DEVICE_PAI_CERT_PATH, CONFIG_CHIP_DEVICE_DAC_CERT_PATH, CONFIG_CHIP_DEVICE_DAC_KEY_PATH must be set to proper values.
For more information see descriptions of kConfigs.")
endif()
...
# in other case user must provide Certs or request to generate them
    if(CONFIG_CHIP_FACTORY_DATA_GENERATE_DAC_CERT)
        # generate a new DAC cert and key
        # TODO - add DAC cert generation using chip-cert 
        message(FATAL_ERROR "DAC cert Generation is not supported yet")
    elseif(CONFIG_CHIP_DEVICE_DAC_CERT_PATH AND CONFIG_CHIP_DEVICE_DAC_KEY_PATH)
        # there is no need to generate new DAC cert/key, just get it from provided file or raw data
        string(APPEND script_args "--dac_cert \"${CONFIG_CHIP_DEVICE_DAC_CERT_PATH}\"\n")
        string(APPEND script_args "--dac_key \"${CONFIG_CHIP_DEVICE_DAC_KEY_PATH}\"\n")
    else()
        message(FATAL_ERROR "DAC cert generation is disable and CONFIG_CHIP_DEVICE_DAC_CERT_PATH or CONFIG_CHIP_DEVICE_DAC_KEY_PATH were not provided")
    endif()
    if(CONFIG_CHIP_FACTORY_DATA_GENERATE_PAI_CERT)
        # generate a new PAI cert
        # TODO - add PAI cert generation using chip-cert 
        message(FATAL_ERROR "PAI cert Generation is not supported yet")
    elseif(CONFIG_CHIP_DEVICE_PAI_CERT_PATH)
        # there is no need to generate new PAI cert, just get it from provided file  or raw data
        string(APPEND script_args "--pai_cert \"${CONFIG_CHIP_DEVICE_PAI_CERT_PATH}\"\n")
    else()
        message(FATAL_ERROR "DAC cert generation is disable and CONFIG_CHIP_DEVICE_PAI_CERT_PATH was not provided")
    endif()
endif()
...
string(APPEND script_args "--passcode ${CONFIG_CHIP_DEVICE_PAIRING_PASSCODE}\n")
...
file(GENERATE
    OUTPUT ${output_path}/${factory_data_target}.args
    CONTENT ${script_args})
...
#
# To use this function a global kConfigs:
# - CONFIG_CHIP_FACTORY_DATA_PARTITION_ADDRESS
# - CONFIG_CHIP_FACTORY_DATA_PARTITION_SIZE
# must be provided.
# 
...
if(NOT CONFIG_CHIP_FACTORY_DATA_PARTITION_ADDRESS OR NOT CONFIG_CHIP_FACTORY_DATA_PARTITION_SIZE)
message(FATAL_ERROR "Can not generate factory_data hex file. Please ensure that CONFIG_CHIP_FACTORY_DATA_PARTITION_ADDRESS and CONFIG_CHIP_FACTORY_DATA_PARTITION_SIZE were provided")
endif()
...
string(APPEND cbor_script_args "--offset ${CONFIG_CHIP_FACTORY_DATA_PARTITION_ADDRESS}\n")
string(APPEND cbor_script_args "--size ${CONFIG_CHIP_FACTORY_DATA_PARTITION_SIZE}\n")
string(APPEND cbor_script_args "-r\n")
# Pass the argument list via file
file(GENERATE
    OUTPUT ${output_path}/${factory_data_target}_cbor.args
    CONTENT ${cbor_script_args})
...
# 
# During generation process a some file will be created in zephyr's build directory:
# - merged_fd.hex a file containing firmware and factory data merged to single file
# - factory_data.hex a file containing only a factory data partition including proper offset
#
# [Args]:
#   target_name         - a name for target to generate factory_data
function(nrfconnect_generate_factory_data target_name)
...
# check if factory data has been already generated
if(EXISTS ${OUTPUT_FILE_PATH}/${target_name}.hex)
    message("\nFactory data was created previously, to generate a new one delete ${target_name}.hex file in your build directory\n")
else()
# create a JSON file with all factory data
nrfconnect_create_factory_data_json(${target_name} 
...
nrfconnect_create_factory_data_hex_file(${target_name} 
...
if(CONFIG_CHIP_MERGE_FACTORY_DATA_WITH_FIRMWARE)                                   
    ...
    add_custom_target(${target_name}_merge
